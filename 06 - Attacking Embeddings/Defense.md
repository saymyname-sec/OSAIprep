...
auditd: execve of python3 by user ts_svc/www-data outside app path
...
type=SYSCALL uid=0 ppid=[ts_svc pid] — unexpected privilege escalation
```
...
**Indicators of Compromise (IoCs):** Multiple NTLM logons across SRV1 and DC01 from same attacking IP. DRSUAPI calls from non-DC machine. `Unattend.xml` access in `C:\Windows\Panther\`.
