...
**DEV domain** — `dev.megacorpone.ai`; child domain with bidirectional trust to DMZ domain; hosts CLIENT01/02, db01.dev, gitlab01.

**DMZ domain** — `dmz.megacorpone.ai`; outer domain containing DB01, WEB01, DC01, MAIL01, CONNECT02.

**GenericWrite** — AD permission allowing modification of an object's attributes, including group membership; abused to add dmzsvc to VPN Users group.
...
**Map-PSDriveCustom.exe** — SSMS plugin binary on db01.dev; contains hardcoded unicode strings including MEGACORPONE\devaccess credentials used to map network drives.

**MEGACORPONE domain** — `megacorpone.ai`; parent/internal domain; final target for Domain Admin.
...
**rag_kb** — MEGACORPONE domain account that runs the health_monitor.exe share-mapping function; credentials recovered from process memory dump.
...
**RD Gateway / CONNECT02** — Remote Desktop Gateway server; tunnels RDP over HTTPS; controlled by Resource Authorization Policies (RAPs) defining which hosts and users are permitted.
...
**SSMS plugin** — SQL Server Management Studio plugin directory; used by devdbsvc to auto-map network drives on startup; Map-PSDriveCustom.exe contains hardcoded parent-domain credentials.
...
**VPN Users group** — DMZ AD group that grants RDS Gateway access to CLIENT01/02 in dev.megacorpone.ai; dmzsvc has GenericWrite, allowing self-addition.
