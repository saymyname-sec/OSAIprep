...
**Resolution:** ✅
Standard pattern (applied on db-host, db01.dev, CLIENT03, CLIENT04):
...
**Resolution:** ✅
- Enumerate jump-host SPNs from AD (`TERMSRV/` = Terminal Services = RD Gateway)
...
- Check svc-web ACL on group → GenericWrite confirmed
- Add svc-web to VPN Users via LDAP `GetDirectoryEntry()` + `CommitChanges()`
- RDP through gateway: `xfreerdp3 /gateway:g:"jump-host..."` with `/drive:` share
...
- Applied to: Map-PSDriveCustom.exe (SSMS plugin) → example-corp\devaccess credentials
...
- Identity: `example-corp\CLIENT04$` (machine account used for network auth under SYSTEM)
...
- Traditional misconfigs (svc-web GenericWrite, svc-db MSSQL sysadmin, devaccess in plugin binary, nora.klein Python automation, lily.fisher writable scheduled task binary) bridge the two AI attack points
...
| # | Item | Status |
|---|------|--------|
| 1 | Exact OS build number for db-host (lab-specific) | Pending lab |
| 2 | web-host exact IP third octet answer | Pending lab |
...
- Credential chain: svc-web → svc-db → devaccess → rag_kb → lily.fisher → administrator
